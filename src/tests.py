...
class TestTextClassifyBaselineMiniTrain(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.trainingFilePath="src/training_files/minitrain.txt"
        self.devFilePath="src/training_files/minidev.txt"
        self.io = EventsIO()
        self.metrics = Metrics()

    def test_GenerateTuplesFromTrainingFile(self):
        # print(f"path: {self.trainingFilePath}")
        examples = self.io.generate_tuples_from_file(self.trainingFilePath)
    ...
    def test_precision(self):
        ...
        self.assertEqual((1 / 2), self.metrics.precision(gold, classified))
    ...
    def test_recall(self):
        ...
        classified = [1, 0, 0, 0, 1]
        classified = [str(b) for b in classified]
        self.assertEqual((1 / 3), self.metrics.recall(gold, classified))
    ...
    def test_f1(self):
        ...
        p = 1 / 2
        r = 1 / 3
        self.assertEqual((2 * p * r) / (p + r), self.metrics.f1(gold, classified))
    ...
    def test_precisionrecallf1together(self):
        ...
        p = 1
        r = 1
        self.assertAlmostEqual(p, self.metrics.precision(gold, classified), places = 4)
        self.assertAlmostEqual(r, self.metrics.recall(gold, classified), places = 4)
        self.assertAlmostEqual((2 * p * r) / (p + r), self.metrics.f1(gold, classified), places = 4)
        ...
        self.assertAlmostEqual(p, self.metrics.precision(gold, classified), places = 4)
        self.assertAlmostEqual(r, self.metrics.recall(gold, classified), places = 4)
        self.assertAlmostEqual(0, self.metrics.f1(gold, classified), places = 4)
